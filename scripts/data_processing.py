## data processing pipeline for brain tumor classification ##

from pathlib import Path
from typing import Tuple, Dict, List
import numpy as np
import torch
from torch.utils.data import Dataset, DataLoader
from torchvision import transforms
from PIL import Image
from sklearn.model_selection import train_test_split
from collections import Counter

from config import (
    RAW_DIR, CLASSES, NUM_CLASSES, BATCH_SIZE, NUM_WORKERS,
    PIN_MEMORY, PERSISTENT_WORKERS, IMAGE_SIZE, MEAN, STD, 
    GRAPHICS_DIR, VAL_SIZE, TEST_SIZE, SPLIT_RANDOM_STATE,
    AUG_SAMPLES_PER_CLASS, ROTATION_DEGREES, GAUSSIAN_BLUR_PROB,
    PREFETCH_FACTOR, DROP_LAST_TRAIN, HORIZONTAL_FLIP_PROB,
    VERTICAL_FLIP_PROB
)


class BrainTumorDataset(Dataset):
    
    def __init__(self, image_paths: List[Path], labels: List[int], transform=None):
        self.image_paths = image_paths
        self.labels = labels
        self.transform = transform
        
    def __len__(self):
        return len(self.image_paths)
    
    def __getitem__(self, idx):
        img_path = self.image_paths[idx]
        label = self.labels[idx]
        image = Image.open(img_path).convert('RGB')
        
        if self.transform:
            image = self.transform(image)
            
        return image, label


def get_train_transforms():
    ## training augmentation ##
    return transforms.Compose([
        transforms.Resize((IMAGE_SIZE, IMAGE_SIZE)),
        
        ## geometric augmentations ##
        transforms.RandomRotation(degrees=ROTATION_DEGREES),
        transforms.RandomAffine(degrees=0, translate=(0.1, 0.1), scale=(0.9, 1.1)),
        transforms.RandomHorizontalFlip(p=HORIZONTAL_FLIP_PROB),
        transforms.RandomVerticalFlip(p=VERTICAL_FLIP_PROB),
        
        ## intensity augmentations ##
        transforms.ColorJitter(brightness=0.2, contrast=0.2, saturation=0.1, hue=0.05),
        transforms.RandomApply([
            transforms.GaussianBlur(kernel_size=3, sigma=(0.1, 1.0))
        ], p=GAUSSIAN_BLUR_PROB),
        
        transforms.ToTensor(),
        transforms.Normalize(mean=MEAN, std=STD),
    ])


def get_val_test_transforms():
    return transforms.Compose([
        transforms.Resize((IMAGE_SIZE, IMAGE_SIZE)),
        transforms.ToTensor(),
        transforms.Normalize(mean=MEAN, std=STD)
    ])


def get_tta_transforms():
    ## test time augmentation transforms ##
    return [
        transforms.Compose([
            transforms.Resize((IMAGE_SIZE, IMAGE_SIZE)),
            transforms.ToTensor(),
            transforms.Normalize(mean=MEAN, std=STD)
        ]),
        transforms.Compose([
            transforms.Resize((IMAGE_SIZE, IMAGE_SIZE)),
            transforms.RandomHorizontalFlip(p=1.0),
            transforms.ToTensor(),
            transforms.Normalize(mean=MEAN, std=STD)
        ]),
        transforms.Compose([
            transforms.Resize((IMAGE_SIZE, IMAGE_SIZE)),
            transforms.RandomVerticalFlip(p=1.0),
            transforms.ToTensor(),
            transforms.Normalize(mean=MEAN, std=STD)
        ]),
        transforms.Compose([
            transforms.Resize((IMAGE_SIZE, IMAGE_SIZE)),
            transforms.RandomRotation(degrees=(5, 5)),
            transforms.ToTensor(),
            transforms.Normalize(mean=MEAN, std=STD)
        ]),
        transforms.Compose([
            transforms.Resize((IMAGE_SIZE, IMAGE_SIZE)),
            transforms.RandomRotation(degrees=(-5, -5)),
            transforms.ToTensor(),
            transforms.Normalize(mean=MEAN, std=STD)
        ]),
    ]


def predict_with_tta(model, image, device, tta_transforms=None):
    ## apply tta during inference ##
    if tta_transforms is None:
        tta_transforms = get_tta_transforms()
    
    model.eval()
    predictions = []
    
    with torch.no_grad():
        for transform in tta_transforms:
            aug_image = transform(image).unsqueeze(0).to(device)
            output = model(aug_image)
            predictions.append(torch.softmax(output, dim=1))
    
    return torch.stack(predictions).mean(dim=0)


def collect_data_paths(data_dir: Path) -> Tuple[List[Path], List[int]]:
    image_paths = []
    labels = []
    
    for class_idx, class_name in enumerate(CLASSES):
        class_dir = data_dir / class_name
        if not class_dir.exists():
            continue
        
        for img_path in class_dir.glob('*'):
            if img_path.suffix.lower() in ['.jpg', '.jpeg', '.png', '.bmp']:
                image_paths.append(img_path)
                labels.append(class_idx)
    
    return image_paths, labels


def analyze_dataset(labels: List[int], split_name: str = "dataset"):
    counter = Counter(labels)
    total = len(labels)
    
    print(f"\n{split_name}:")
    print(f"  total: {total}")
    for class_idx, class_name in enumerate(CLASSES):
        count = counter[class_idx]
        percentage = (count / total) * 100
        print(f"  {class_name}: {count} ({percentage:.2f}%)")
    
    return counter


def create_splits(
    image_paths: List[Path],
    labels: List[int],
    val_size: float = VAL_SIZE,
    test_size: float = TEST_SIZE,
    random_state: int = SPLIT_RANDOM_STATE
) -> Dict[str, Tuple[List[Path], List[int]]]:
    
    train_val_paths, test_paths, train_val_labels, test_labels = train_test_split(
        image_paths, labels, test_size=test_size, stratify=labels, random_state=random_state
    )
    
    val_size_adjusted = val_size / (1 - test_size)
    train_paths, val_paths, train_labels, val_labels = train_test_split(
        train_val_paths, train_val_labels, test_size=val_size_adjusted, 
        stratify=train_val_labels, random_state=random_state
    )
    
    splits = {
        'train': (train_paths, train_labels),
        'val': (val_paths, val_labels),
        'test': (test_paths, test_labels)
    }
    
    print("\ndata splits:")
    analyze_dataset(train_labels, "train")
    analyze_dataset(val_labels, "val")
    analyze_dataset(test_labels, "test")
    
    return splits


def save_augmented_samples(
    dataset: BrainTumorDataset,
    num_samples_per_class: int = AUG_SAMPLES_PER_CLASS,
    graphics_dir: Path = GRAPHICS_DIR
):
    graphics_dir.mkdir(parents=True, exist_ok=True)
    
    class_indices = {class_idx: [] for class_idx in range(NUM_CLASSES)}
    for idx, label in enumerate(dataset.labels):
        class_indices[label].append(idx)
    
    print(f"\nsaving augmented samples to {graphics_dir}")
    
    for class_idx, class_name in enumerate(CLASSES):
        if len(class_indices[class_idx]) == 0:
            continue
        
        sample_idx = class_indices[class_idx][0]
        
        for i in range(num_samples_per_class):
            aug_img, _ = dataset[sample_idx]
            
            ## denormalize ##
            aug_img_display = aug_img.clone()
            for t, m, s in zip(aug_img_display, MEAN, STD):
                t.mul_(s).add_(m)
            aug_img_display = torch.clamp(aug_img_display, 0, 1)
            
            aug_img_np = aug_img_display.permute(1, 2, 0).numpy()
            aug_img_np = (aug_img_np * 255).astype(np.uint8)
            aug_img_pil = Image.fromarray(aug_img_np)
            
            filename = f"{class_name}_aug_{i+1}.png"
            aug_img_pil.save(graphics_dir / filename)
    
    print("done")


def create_dataloaders(
    splits: Dict,
    batch_size: int = BATCH_SIZE,
    num_workers: int = NUM_WORKERS
) -> Tuple[Dict[str, DataLoader], Dict[str, Dataset]]:
    
    datasets = {
        'train': BrainTumorDataset(splits['train'][0], splits['train'][1], transform=get_train_transforms()),
        'val': BrainTumorDataset(splits['val'][0], splits['val'][1], transform=get_val_test_transforms()),
        'test': BrainTumorDataset(splits['test'][0], splits['test'][1], transform=get_val_test_transforms())
    }
    
    dataloaders = {
        'train': DataLoader(
            datasets['train'],
            batch_size=batch_size,
            shuffle=True,
            num_workers=num_workers,
            pin_memory=PIN_MEMORY,
            persistent_workers=PERSISTENT_WORKERS if num_workers > 0 else False,
            drop_last=DROP_LAST_TRAIN,
            prefetch_factor=PREFETCH_FACTOR if num_workers > 0 else None
        ),
        'val': DataLoader(
            datasets['val'],
            batch_size=batch_size,
            shuffle=False,
            num_workers=num_workers,
            pin_memory=PIN_MEMORY,
            persistent_workers=PERSISTENT_WORKERS if num_workers > 0 else False,
            prefetch_factor=PREFETCH_FACTOR if num_workers > 0 else None
        ),
        'test': DataLoader(
            datasets['test'],
            batch_size=batch_size,
            shuffle=False,
            num_workers=num_workers,
            pin_memory=PIN_MEMORY,
            persistent_workers=PERSISTENT_WORKERS if num_workers > 0 else False,
            prefetch_factor=PREFETCH_FACTOR if num_workers > 0 else None
        )
    }
    
    print("\ndataloaders created")
    print(f"  train batches: {len(dataloaders['train'])}")
    print(f"  val batches: {len(dataloaders['val'])}")
    print(f"  test batches: {len(dataloaders['test'])}")
    
    return dataloaders, datasets


def process_and_prepare_data(
    raw_data_dir: Path = RAW_DIR,
    val_size: float = VAL_SIZE,
    test_size: float = TEST_SIZE
) -> Tuple[Dict[str, DataLoader], Dict[str, Dataset]]:
    
    print("\ncollecting images...")
    image_paths, labels = collect_data_paths(raw_data_dir)
    print(f"found {len(image_paths)} images across {NUM_CLASSES} classes")
    
    if len(image_paths) == 0:
        raise ValueError(f"no images found in {raw_data_dir}")
    
    splits = create_splits(image_paths, labels, val_size, test_size)
    dataloaders, datasets = create_dataloaders(splits)
    
    print(f"\ntrain: {len(splits['train'][0])}")
    print(f"val: {len(splits['val'][0])}")
    print(f"test: {len(splits['test'][0])}")
    
    return dataloaders, datasets


if __name__ == "__main__":
    dataloaders, datasets = process_and_prepare_data()
    save_augmented_samples(datasets['train'])
    
    train_loader = dataloaders['train']
    images, labels = next(iter(train_loader))
    
    print(f"\nbatch shape: {images.shape}")
    print(f"labels shape: {labels.shape}")
    print("pipeline ready")